 ...
 * Revision 1.2 - Added overheat handling
 */
...
bool b_power_trending_up = true; //Tracks whether wand power is increasing
...
void powerMeterConfig() {
  DEBUG_PRINTLN(F("Configuring Power Meter"));
...
void powerMeterInit() {
  ...
  DEBUG_PRINT(F("Power Meter Initialization Result: "));
  DEBUG_PRINTLN(i_monitor_status);
  ...
  if(i_monitor_status == 0) {
    ...
    DEBUG_PRINTLN(F("Unable to find power monitoring device on i2c."));
  }
}
...
void updateWandPowerState() {
  ...
  if (b_wand_overheated) {
    ...
    if (ms_delay_post_2.justFinished()) {
      DEBUG_PRINTLN("Wand Stopped Firing (Overheat Sequence Finished)");
      wandStoppedFiring();
      b_wand_firing = false; 
    ...
    if (PACK_STATUS == MODE_OFF) {
      ...
      b_skip_next_power_on_pulse = false;
    ...
    if (!b_wand_on) {
      ...
      f_accumulator = 0.0;
      if (f_diff_average > POWER_ON_THRESHOLD_FAST || 
        (f_diff_average > POWER_ON_THRESHOLD_SLOW_AVG && f_on_average > POWER_ON_MIN_IDLE_POWER)) {
      ...
      if ((f_diff_average < POWER_OFF_THRESHOLD_FAST && f_sliding_window[19] < POWER_OFF_MAX_POWER) || f_sliding_window[19] < POWER_OFF_ABSOLUTE_MAX) {
        ...
        if (!b_wand_firing) {
          ...
          //Original code commented out and changed to track upward trending
          ...
          b_power_trending_up = (f_diff_average > 0); // Only allow firing if power is rising

          if (b_power_trending_up) {
              // START FIRING LOGIC
              for(uint8_t i = 9; i < 19; i++) { f_accumulator += (f_sliding_window[i + 1] - f_sliding_window[i]); }
              f_diff_average = f_accumulator / 10.0;
              f_accumulator = 0.0;
              ...
              if ((f_diff_average > FIRING_START_AVG_RATE_MIN && f_diff_average < FIRING_START_AVG_RATE_MAX) ||
                  (f_range > FIRING_START_POWER_JUMP)) {
                  ms_delay_post_2.start(i_wand_overheat_delay); // Start the 14.5s overheat timer
                  f_idle_value = f_sliding_window[9];
                  wandFiring();
                  b_wand_firing = true;
...
void wandPowerDisplay() {
  if(b_show_power_data) {
    DEBUG_PRINT(F("W.Raw(W):"));
    DEBUG_PRINT(wandReading.RawPower);
    DEBUG_PRINTLN(F(","));
  }
}
