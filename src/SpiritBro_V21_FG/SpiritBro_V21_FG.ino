...
const unsigned long DEBUG_PRINT_INTERVAL = 5000; // Print state debug every 5 seconds
...
uint32_t packStartupTimer = 0;
const uint32_t PACK_STARTUP_DELAY = 9000; // 9 second startup delay
...
void setup() {
  ...
  DEBUG_PRINTLN(firingSoundEnabled);
  DEBUG_PRINTLN("");
...
void loop() {
    ...
    switch(PACK_STATUS) {
        case MODE_POWERDOWN:
            ...
            delay(200); //Add a 200ms delay before sending the power pulse just in case we came off of a firing pulse to avoid a vent.
            sendPowerPulse();
            DEBUG_PRINTLN("State: MODE_OFF - Pack is now off");
            break;
        case MODE_BOOTING:
            // Check if we're still within startup window
            if (currentMillis - packStartupTimer >= PACK_STARTUP_DELAY) {
                // Startup complete
        ...
        case WAND_FIRING:
        case MODE_OFF:
            // No state-specific action needed
            break;
    }

    // Periodically print state for debugging
    #if DEBUG == 1
    if (currentMillis - lastDebugPrintMillis >= DEBUG_PRINT_INTERVAL) {
        lastDebugPrintMillis = currentMillis;
        DEBUG_PRINT("Current State: ");
        DEBUG_PRINTLN(PACK_STATUS);
    }
    #endif
...
void pulseFireButton(unsigned long duration) {
  pulsePin(FIRE_BTN_PIN, duration);
}

void wandFiring(){
  if(firingSoundEnabled == true){
...
void updateStatusLED(uint32_t now) {
  ...
  switch (PACK_STATUS) {
    ...
    case WAND_STOPPED_FIRING:
      ...
      break;
    case MODE_OFF:
      speed = intPicoLEDSpeed;
      color = CRGB::Green;
      break;
  }

  if (speed > 0) {
    blinkPicoLED(speed, color, now);
  } else {
...
//Blinking the status LED on the PCB - pass currentMillis to avoid redundant millis() calls
void blinkPicoLED(uint32_t intPicoLEDSpeed, CRGB picoLEDColor, uint32_t now) { 
  if (now - previousPicoLEDMillis >= intPicoLEDSpeed) { 
    previousPicoLEDMillis = now; 
    ledOn = !ledOn; 
...
void buttonPressHandler() {
  // Long Press - Enter Music Mode
  if (functionButton.longPress()) {
    musicMode = !musicMode;
    DEBUG_PRINT("Long Press -> Music Mode: ");
    DEBUG_PRINTLN(musicMode ? "ON" : "OFF");
    ...
    if (functionButton.singleClick()) {
      ...
      if(musicMode){
        DEBUG_PRINTLN("Single-Click - In Music Mode - Skipping Track");
        pulsePin(FIRE_BTN_PIN, 300);
      } else {
        DEBUG_PRINTLN("Single-Click - Normal Mode - Sending Power Pulse");  
        sendPowerPulse();
      }

    }

    // Double-Click - Manual Vent
    if (functionButton.doubleClick()) {
      DEBUG_PRINTLN("Double-Click - Manual Vent Request ");

        //Only send the vent button combo if the pack is running
        if (PACK_STATUS != MODE_OFF) {
          DEBUG_PRINTLN("Pack is running - Sending Vent Request");
          digitalWrite(PWR_BTN_PIN, LOW);
          digitalWrite(FIRE_BTN_PIN, LOW);
          delay(PWRPULSE_LENGTH);
          digitalWrite(PWR_BTN_PIN, HIGH);
          digitalWrite(FIRE_BTN_PIN, HIGH);
      } else {
        DEBUG_PRINTLN("Pack is off - Ignoring Vent Request");
      }
    }  
}
